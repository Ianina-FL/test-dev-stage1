...
const CardUsers = ({
  ...
  const hasUpdatePermission = hasPermission(currentUser, 'UPDATE_ITEMS');
          ...
          items.map((item, index) => (
            ...
            >
              ...
              >
                ...
                <div className='ml-auto '>
                  <ListActionsPopover
                    ...
                    onEdit={onEdit}
                    itemId={item.id}
                    pathEdit={`/items/items-edit/?id=${item.id}`}
                    pathView={`/items/items-view/?id=${item.id}`}
              ...
              <dl className='divide-y  divide-stone-300   dark:divide-dark-700 px-6 py-4 text-sm leading-6 h-64 overflow-y-auto'>
                ...
                <div className='flex justify-between gap-x-4 py-3'>
                  <dt className='  text-gray-500  dark:text-dark-600'>Size</dt>
                  <dd className='flex items-start gap-x-2'>
                    <div className='font-medium line-clamp-4'>{item.size}</div>
                  </dd>
                </div>
              </dl>
            </li>
